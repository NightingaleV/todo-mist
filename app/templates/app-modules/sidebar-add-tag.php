<div class="collapse multi-collapse" id="addTag">
  <div class="card card-body">
    <form class="addTag-form" method="post" autocomplete="off">
      <div class="addTag-container input-group">
        <input name="tag" type="text" class="addTag-input form-control" placeholder="New tag"
               aria-label="New task" aria-describedby="basic-addon2"
               data-toggle="popover" data-placement="top" data-content="This tag already exist.">
        <div class="addTag-btn-box input-group-append">
          <button class="addTag-btn  btn btn-secondary" type="submit">
            <i class="addTag-icon material-icons">add</i>
          </button>
        </div>
      </div>
    </form>
  </div>
</div>