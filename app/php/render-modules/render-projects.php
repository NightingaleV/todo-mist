...
if (isset($_SESSION['id'])) {
    ...
    while ($rows = $result->fetch_assoc()) {
        ...
        $project_id = $rows['id'];
        $project= htmlentities($rows['project'], ENT_QUOTES, 'UTF-8');
        $project_position = $rows['project_position'];

        $response .= '<li class="project-item list-group-item" data-project-position="' . $project_position . '" data-project-id="' . $project_id . '">';
        $response .= '<div class="project-left-wrapper">';
        $response .= $project_left_controls;
        $response .= '<span class="project-label">' . $project . '</span></div>';
